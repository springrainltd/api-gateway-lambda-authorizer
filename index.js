...
AuthPolicy.prototype = (function AuthPolicyClass() {
  ...
  function addMethod(effect, verb, resource, conditions) {
    ...
    if (!this.pathRegex.test(resource)) {
      throw new Error(
        `Invalid resource path: ${resource}. Path should match ${
          this.pathRegex
        }`
      );
    }
    ...
    const resourceArn = `arn:aws:execute-api:${this.region}:${
      this.awsAccountId
    }:${this.restApiId}/${this.stage}/${verb}/${cleanedResource}`;
...
exports.handler = async (event, context, callback) => {
  try {
    ...
    // const response = Object.assign(authResponse, {userid: "test123456"});
